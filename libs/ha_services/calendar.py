...
class Calendar():
    ...
    def to_text(self):
        if(len(self.events) == 0): return f"{self.calendar_owner} doesn't have near events"
        final_text = f"{self.calendar_owner} events: ["
        for event in self.events:
            happening_text = self.get_happening_text(event)
            if("Past event" != happening_text):
                final_text += f"\n- {event.to_text()}"
        final_text += "]"
        return final_text
...
class Event():
    ...
    def to_text(self):
        ...
        day_output_format="%A, %b %d"
        same_day_end_time = "%I:%M%p"

        start_day = self.start_time.strftime(day_output_format)
        end_day = self.end_time.strftime(day_output_format)
        if(self.start_time == self.end_time):
            time = self.start_time.strftime("%A %b %d")
        elif(start_day == end_day):
            start_time = self.start_time.strftime(datetime_output_format)
            end_time = self.end_time.strftime(same_day_end_time)
